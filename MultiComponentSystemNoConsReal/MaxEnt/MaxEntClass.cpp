...
MaxEnt::MaxEnt(const std::string& dataFile, const std::string& configFile)
{
    ...
    ReadFile(configFile, _totalIterations, _maximumSearchIter, _numBasisVectors, _zeroLevel, _minVar, _dataScale, _numSubstances, _propagationSLD,
     _substrateSLD, _lengthPropagation, _lengthSubstrate, _smoothIncrement, _useEdgeConstraints, _chiSquaredScale, _spikeCharge, _spikePortion, _spikeAmount,
      ...
      _initChargeSpecific, _noise, _initLayerStart, _initLayerStop, _lengthPropagationReal, _lengthSubstrateReal, _qOffsetReal, _qCutOffReal, _initLayerStartReal, _initLayerStopReal, _realWorldScaling, _knownSubstrate);
    ...
    if(_initChargeSpecific == true)
    {
        //bounds check for initLayerStop/Start
        for(int i = 0; i < _numSubstances; i++)
        {
            if(_initLayerStop[i] > _depth)
            {
                std::cout << "initLayerStop is too large, defaulting to maximum range" << std::endl;
                std::cout << "Press X to quit or any other button to continue with the simulation" << std::endl;
                char answer;
                std::cin >> answer;
                if(answer == 'X' || answer == 'x')
            ...
            if(_initLayerStart[i] < 0)
            {
                std::cout << "initLayerStart is too small, defaulting to maximum range" << std::endl;
                std::cout << "Press X to quit or any other button to continue with the simulation" << std::endl;
                char answer;
                std::cin >> answer;
                if(answer == 'X' || answer == 'x')
    ...
    //set values below _qOffset to the value at _qOffset
    for(int i = 0; i < _depth; i++)
    {
        if(i < _qOffset)
        {
    ...
    if(_realWorldScaling == true)
    {
        for(int i = 0; i < _numSubstances; i++)
        {
            _RealToIndex(_sldScale, _initLayerStopReal[i], _initLayerStop[i]);
            _RealToIndex(_sldScale, _initLayerStartReal[i], _initLayerStart[i]);
        }

        if(_knownSubstrate != true)
        {
    ...
    //load in substrate profile if known
    if(_knownSubstrate == true)
    {
        arma::mat substrateProfile;
        substrateProfile.load("substrate"); //get rid of this hard coding

        _substrateProfile = substrateProfile.col(1);
        _lengthSubstrate = _substrateProfile.n_elem; 
    }
...
void MaxEnt::_Init()
{
    ...
    **/
    ...
    if(_initChargeSpecific == true)
    {
        for(int i = 0; i < _numSubstances; i++)
        {
            _InitChargeSpecific(_charge[i], _total[i], _initLayerStart[i], _initLayerStop[i]);
        }
    }
    else
    {
        for(int i = 0; i < _numSubstances; i++)
        {
            _InitChargeRand(_charge[i], _total[i]);
        }
    }
 ...
 */
void MaxEnt::_RealToIndex(arma::vec& scale, double& value, int& index)
{
 ...
 * @param index the index 
 */
void MaxEnt::_RealToIndexLength(arma::vec& scale, double& value, int& index)
{
...
arma::mat MaxEnt::GetSLDScaled()const
{
    ...
    if(_boundSLD == true|| _volumetricNormalisation == true)
    {
        sldTemp *= _reScaling;
    }
...
void MaxEnt::_ChiSquared(double& sld)
{
    ...
    //false SLD, which takes the sign of the SLD
    //for chi-squared grad direction
    double e = 1.0*sgn(sld);
    ...
    _gradChiSquared = e*_chiSquaredScale*_gradChiSquared;
 ...
 * This initialisation works off a random dist
 * 
 * @param charge The charge of one of the components of the system
 * @param norm The normalisation of the component of the system
 */
void MaxEnt::_InitChargeRand(arma::vec& charge, double& norm)
{
...
/**
 * @brief Class member function to initialise the charge
 * this initialisation takes in a specified start distribution
 * 
 * @param charge The charge of one of the components of the system
 * @param norm The normalisation of the component of the system
 * @param initLayerStart The index of where this component begins
 * @param initLayerStop The index of where this component ends
 */
void MaxEnt::_InitChargeSpecific(arma::vec& charge, double& norm, int& initLayerStart, int&initLayerStop)
{
    //If specified start with a noisy distribution, otherwise a clean distribution is used;
    if(_noise == true)
    {
        //randomly seed the arma random number generator 
        arma::arma_rng::set_seed_random();
        ...
        for(int i = 0; i < _depth; i++)
        {
            if(i >= initLayerStart && i <= initLayerStop)
            {
                charge[i] += _spikeAmount;
            }
        }
    }
    else
...
void MaxEnt::_SLDGenerate()
{
    ...
    if(_useEdgeConstraints == true)
    {
        ...
        {
            _sld.tail(_lengthSubstrate) = _substrateProfile;
        }
    }


    double totalTemp =  fabs(accu(_sld));

    _Renormalise(_sld, _norm);

    _reScaling = totalTemp/_norm;
...
void MaxEnt::_DiagH(double& sld)
{
    ...
    for(int i = 0; i < _numBasisVectors; i++)
	{
        ...
        //get 1/j^2 scaling and set _qOffset region
        for(int j = 0; j < _depth; j++)
        {
            eVecData[i][j] /= pow(_deltaQ*j, 2.0);
        }
...
void MaxEnt::_ForceSLD()
{
    ...
    int chargeIndexMax = totalCharge.index_max();

    int index;

    //force values below the threshold to be 0
    for(int i = chargeIndexMax; i < _depth; i++)
    {
        if(totalCharge[i] < 0.05*chargeMax)
        {
            index = i; 
            _lengthSubstrate = (_depth - index) + 1;
            break;
        }
    }
}
