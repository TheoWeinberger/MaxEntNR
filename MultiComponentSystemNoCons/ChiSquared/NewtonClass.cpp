...
Newton::Newton(const std::string& dataFile)
{
    ...
    //data outside of qOFfset and qCutOff is cropped
    for(int i = 0; i < _depth; i++)
    {
        ...
        {
            _dataFit[i] = _dataFit[i]*pow((i*_deltaQ), 4.0);
        }
        else if(i >= _qCutOff)
        {
            _dataFit[i] = 0.0;
    ...
    //Regions outside of qOffset/CutOff are set to the boundary values
...
Newton::Newton(const std::string& dataFile, const std::string& configFile)
{
    ...
    //output intial data for reference
...
arma::mat Newton::GetReflectivityScaled()const
{
    ...
    for(int i = 0; i < _depth; i++)
    {
        ...
        {
            reflectivityScaled(i,1) = 1.0;
            normalisation = _sldImage[i];
        }
        else 
        {
            reflectivityScaled(i,1) = _sldImage[i]/(normalisation*i*i*i*i);
        }
    }
